 ...
 * busca no topo. Aceita texto livre. Opcional swatch de cor.
 */
export default function Combo({ value, onSelect, options, placeholder, leftSwatch, allowFree = true }: {
  value: string; onSelect: (v: string, opt?: ComboOpt) => void; options: ComboOpt[]
  placeholder?: string; leftSwatch?: string; allowFree?: boolean
}) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button type="button" onClick={() => { setOpen(o => !o); setQ('') }} style={control}>
        ...
        <span style={{ flex: 1, textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: value ? 'var(--text)' : 'var(--text-subtle)' }}>{value || placeholder}</span>
...
const control: CSSProperties = { display: 'flex', alignItems: 'center', gap: 8, width: '100%', height: 'var(--control-h-lg)', padding: '0 10px', border: '1px solid var(--border-strong)', borderRadius: 8, background: 'var(--bg-surface)', color: 'var(--text)', font: 'inherit', fontSize: 14, cursor: 'pointer' }
