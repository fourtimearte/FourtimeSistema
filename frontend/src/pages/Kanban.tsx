...
/* =====================================================================
   ...
   clicável abre o orçamento HTML; Mover no cabeçalho; módulos DA FATIA.
   ===================================================================== */
function CardModal({ card, pedido, onClose, onMove, onEditor }: {
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 80, display: 'grid', placeItems: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', width: 880, maxWidth: '100%', maxHeight: '94vh', overflowY: 'auto', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderTop: '4px solid ' + cor, borderRadius: 12, boxShadow: 'var(--sh-4)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg-surface)', zIndex: 2 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={() => setVerOrc(v => !v)} title="Abrir o orçamento (versão HTML) aqui dentro"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto' }}>
            <select value={card.station} onChange={e => onMove(e.target.value)} title="Mover para"
              style={{ height: 32, maxWidth: 190, padding: '0 8px', borderRadius: 8, font: 'inherit', fontSize: 12, background: 'var(--bg-surface-2)', color: 'var(--text)', border: '1px solid var(--border-strong)', cursor: 'pointer' }}>
        ...
        <div style={{ padding: '16px 20px' }}>
          {/* orçamento embutido (aberto pelo título) */}
          {verOrc && (
            <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
              ...
              <iframe title="orçamento" srcDoc={html} style={{ width: '100%', height: 520, border: 'none', display: 'block', background: '#EEF1F4' }} />
            </div>
          )}
...
function PedidoModal({ p, cards, onClose, onLocate, onEditor }: {
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 80, display: 'grid', placeItems: 'center', padding: 16 }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(14,17,22,.55)' }} />
      <div style={{ position: 'relative', width: 880, maxWidth: '100%', maxHeight: '94vh', overflowY: 'auto', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderTop: '4px solid ' + (p.late ? 'var(--alert)' : 'var(--set-comercial)'), borderRadius: 12, boxShadow: 'var(--sh-4)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg-surface)', zIndex: 2 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontWeight: 700, fontSize: 15 }}>{p.cliente || 'Orçamento'} <span className="mono">· {p.pedido}</span></span>
